...
interface ChannelFormData extends Channel {
...
const ChannelDetailEdit = () => {
  ...
  useEffect(() => {
    ...
    const fetchChannel = async () => {
      ...
      try {
        ...
        form.setFieldsValue(channel as ChannelFormData);
      } catch (error) {
  ...
  const onFinish = async (values: ChannelFormData) => {
    if (!channelId) return;
    
    setLoading(true);
    try {
      await channelService.updateChannel(channelId, values as unknown as Record<string, unknown>);
